...
### TODO
市级总值班室：涉及菜单页：信息接报、抄报事件、上报事件、信息签收；区县级总值班室：涉及菜单页：接报信息、信息签收；时间轴事发定位旁边需要加上【转发定位到成都】按钮，点击之后做如下操作：
1. 在当前时间轴上增加一条操作记录：已向成都转发事发定位，记录操作时间、操作单位基本信息。
2. 将该按钮置灰禁用，按钮文本改为【已转发定位到成都】。

需求不明确你可以继续向我追问


我看了下之前的逻辑，在信息签收页面，「转发定位到成都」这个按钮已经存在了，但是出现的条件很狭窄：签收详情时间轴上，只有「来自基层」的那几条带定位的记录旁边才有「转发定位到成都」；市级自己写的定位条旁边没有。 而且点击「转发定位到成都」这个按钮，会产生一条续报记录