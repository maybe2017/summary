...
2. 用于判断某个输入参数 `一定不存在` 或者 `可能存在`。[hash冲突, 待查询的输入，经过hash后的v，可能v下标处已经被标记为1]。
    ...
    bitMap.contains(k) = true, 仅表示这个k可能存在（hash冲突），此时需要去数据库查询，确定到底存不存在【二次确认机制】

    // 短期窗口的去重业务：使用布隆过滤器+redis
    通常二次确认机制使用redis，当判断元素可能存在时，去redis再查一遍，如果存在，则说明业务已经处理过了（达到去重效果）；如果不存在，那么就开始处理业务，且向redis写入当前元素，
    通常携带一个过期时间。
    ```
