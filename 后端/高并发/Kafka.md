...
2. RocketMQ中有MessageQueue，Kafka也有Partition
    ...
    把一个分区就理解为一个队列。一个topic下的消息被key路由到多个分区中，每个分区的消息只能被一个消费者组内的一个消费者消费。【并行度受限】
    ```
3. 消费者组，两者都是 `一个分区的消息只被组内一个消费者消费` 的特性；而且 `消费并行度` 都是由队列（Partition）数量决定！！
    ```
    // 怎么理解消费者组？
    把它理解为单独的一个个下游系统

    // 消息积压时，不能说扩展节点增加消费者组，来增加消费速度，因为消费者数量最多 = 分区数量。
    要提高消费的并发度，那么得先增加物理分区。【会让消息重新负载均衡到各个分区上，增大了Broker的内存压力】

    // 解决方案：
    1. share group共享组(消费者组)：组内多个消费者可以同时从同一个分区里拉取消息
    2. 虚拟分区：自己在消费者逻辑里面实现，按orderId分组消息，然后用不同的线程跑不同的orderId下的消息。
...
2. 消息路由机制（消息分配策略）
    ...
    // 使消息局部有序，即仅保证业务链路消息有序：如key设置为orderId，那么
    轮询策略
    自定义实现
...
### 为啥快【mmap + sendfile】
数据全程不进入 Java 堆，不做多余拷贝，不触发 GC，只靠操作系统内核高速搬运，所以能轻松几十万 TPS。。

① 生产者 → Broker（写入，mmap 加速）
生产者网络包 → Broker 内核 Socket → 直接 mmap 写入页缓存 → 异步刷回磁盘 log（不进 Java 堆，无拷贝）
② 消费者 ← Broker（读取，零拷贝）
磁盘 log → 内核页缓存 → transferTo → Socket 缓冲区 → 消费者（Broker 完全不碰数据，0 次用户态拷贝）【log文件已经在 内核页缓存 里】